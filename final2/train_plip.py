...
class PLIPModel(nn.Module):
    ...
    def __init__(self, embed_dim=512, temperature=0.07):
        ...
        # LiT strategy: FREEZE DINOv3 — preserve its strong SSL features
        # The text encoder will adapt to DINOv3's embedding space, not the other way around
        for param in self.vision_encoder.parameters():
            param.requires_grad = False
        print("   ✓ DINOv3 frozen (LiT strategy)")

        # DINOv3 ViT-B/16 outputs 768-dim features
        vision_width = 768

        # MLP projection: 768 → 768 → 512 (more capacity to bridge DINOv3 → CLIP text space)
        # LayerNorm stabilizes training when vision encoder is frozen
        ...
        # LiT strategy: text encoder is TRAINABLE — it adapts to DINOv3's embedding space
        # text_projection_clip is also trainable (it's a parameter tensor, always trainable)
        print("   ✓ Text encoder trainable (LiT strategy: adapts to DINOv3 space)")
...
def main():
    ...
    # Linear warmup scheduler: ramp LR from 0 → base_lr over warmup_steps
    ...
    print(f"\nStarting training for {args.max_steps} steps (LiT + grad accum + warmup)...")
    print(f"  Dataset size: {len(train_dataset)} samples")
    print(f"  Batch size: {args.batch_size} (x{args.grad_accum_steps} accum = {effective_batch} effective)")
    ...
    while global_step < args.max_steps:
        ...
        for accum_i in range(args.grad_accum_steps):
            ...
            with autocast():
                image_embeddings, text_embeddings, logit_scale = model(images, text_tokens)
                loss = contrastive_loss(image_embeddings, text_embeddings, logit_scale)
                loss = loss / args.grad_accum_steps  # Scale loss for accumulation
        ...
        # Update progress bar
        pbar.update(1)
        pbar.set_postfix({'loss': accum_loss, 'step': global_step, 'lr': scheduler.get_last_lr()[0]})
